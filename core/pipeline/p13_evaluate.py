...
{{
  "summary": "이 영상에서 가장 개선 효과가 클 1가지만 작성. 영상 요약(무엇을/어떻게)을 반복하지 마세요. 반드시 정량 근거(블록 비율, 변화량, 시간 등)를 포함하세요. 1~2문장.",
  "positioning": {{
...
3. 모든 코칭 항목에 반드시 포함:
   ...
   - 다른 구간 또는 영상 평균과의 비교
4. 절대 금지하는 문장 패턴 (마케터가 이미 아는 일반론):
   - "~하는 것이 중요합니다"
   - "~하면 효과적입니다"
   - "~하는 방법입니다"
   - "~를 높이는 데 효과적입니다"
   - "~하면 좋습니다"
   이런 패턴이 포함된 문장은 작성하지 마세요.
5. 제안은 구체적 행동으로 (시간+블록타입 명시):
